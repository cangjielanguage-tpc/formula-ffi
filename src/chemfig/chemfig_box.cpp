...
namespace {
    ...
    float computeShortening(float dirX, float dirY, float halfW, float halfH) {
        float tx = (std::abs(dirX) > EPSILON) ? halfW / std::abs(dirX) : LARGE_FLOAT_VALUE;
        float ty = (std::abs(dirY) > EPSILON) ? halfH / std::abs(dirY) : LARGE_FLOAT_VALUE;
    ...
    TextSegment buildSegment(const std::wstring& text, TextSegmentType type,
        ...
        seg.width = (bounds.w + bounds.x + BOX_TEXT_WIDTH_EXTRA) * textScale * scale;
        ...
        while (*p != L'\0') {
            if (*p == L'\\') {
                ...
                if (cmdName == L"ominus") {
                    ...
                    std::wstring baseArg = ChemfigParser::parseBraceArg(p);
                    std::wstring labelArg = ChemfigParser::parseBraceArg(p);
                    if (!baseArg.empty() || !labelArg.empty()) {
                    ...
                    result += std::wstring(cmdStart, p - cmdStart);
                }
            } else {
                result += *p;
                p++;
...
void ChemfigBox::calculateLayout() {
    ...
    float textScale = BOX_TEXT_SCALE_BASE * _sizeFactor;
...
void ChemfigBox::drawMolecule(Graphics2D& g2, float x, float y) {
    ...
    for (const auto& bond : _molecule.bonds) {
        ...
        if (bondLen > EPSILON) {
            ...
            if (bond.params.hasOffset) {
                float startGap = bond.params.offsetStart * scale * BOX_BOND_OFFSET_SCALE;
                float endGap = bond.params.offsetEnd * scale * BOX_BOND_OFFSET_SCALE;
                fromX += dirX * startGap;
                fromY += dirY * startGap;
                toX -= dirX * endGap;
                toY -= dirY * endGap;
    ...
    for (const auto& ring : _molecule.rings) {
        if (ring.hasInnerCircle && ring.sides > 0 && ring.radius > EPSILON) {
            ...
            float r = apothem * BOX_INNER_CIRCLE_RADIUS_RATIO;
            if (r > EPSILON) {
    ...
    float textScale = BOX_TEXT_SCALE_BASE * _sizeFactor;
    float chargeRadius = BOX_CHARGE_RADIUS_FACTOR * scale * textScale * BOX_CHARGE_DISPLAY_BASE;
    ...
    for (int i = 0; i < static_cast<int>(_molecule.atoms.size()); i++) {
        ...
        for (const auto& charge : atom.charges) {
            ...
            if (charge.mark == L"\"" || charge.mark == L"\\\"") {
                float rectW = BOX_CHARGE_RECT_WIDTH_FACTOR * scale * textScale * BOX_CHARGE_DISPLAY_BASE;
                float rectH = BOX_CHARGE_RECT_HEIGHT_FACTOR * scale * textScale * BOX_CHARGE_DISPLAY_BASE;
                ...
                float lineLen = BOX_CHARGE_LINE_LENGTH_FACTOR * scale * textScale * BOX_CHARGE_DISPLAY_BASE;
                ...
                float dotSize = BOX_CHARGE_DOT_SIZE_FACTOR * scale * textScale * BOX_CHARGE_DISPLAY_BASE;
                float dotSpacing = BOX_CHARGE_DOT_SPACING_FACTOR * scale * textScale * BOX_CHARGE_DISPLAY_BASE;
                float perpAngle = CHEM_PI / 2 - angleRad;
                ...
                float radius = BOX_CHARGE_CIRCLE_RADIUS_FACTOR * scale * textScale * BOX_CHARGE_DISPLAY_BASE;
                g2.drawEllipse(chargeX - radius, chargeY + radius, radius * 2, radius * 2, radius, radius, BOND_LINE_WIDTH * scale, 0.0f);
                float lineLen = radius * BOX_CHARGE_CIRCLE_LINE_RATIO;
...
void ChemfigBox::draw(Graphics2D& g2, float x, float y) {
    ...
    float fromBondExtra = 0.0f, toBondExtra = 0.0f;
        ...
        {
            float fromExtra = fromIsAtom ? BOX_CURVE_FROM_ATOM_EXTRA : fromBondExtra;
            ...
            if (startShorten > 0.0f) {
                ...
                if (len > 0.001f) {
                    float s = startShorten * scale * BOX_CURVE_SHORTEN_SCALE;
                    fromPos.x += dx / len * s;
                    fromPos.y += dy / len * s;
                }
            }
        ...
        {
            float toExtra = toIsAtom ? BOX_CURVE_TO_ATOM_EXTRA : toBondExtra;
            ...
            if (endShorten > 0.0f) {
                ...
                float len = std::sqrt(dx * dx + dy * dy);
                if (len > 0.001f) {
                    float s = endShorten * scale * BOX_CURVE_SHORTEN_SCALE;
                    toPos.x -= dx / len * s;
                    toPos.y -= dy / len * s;
                }
            }
        ...
        if (curve.hasArrow) {
            ...
            float ax1 = toPos.x - arrowSize * std::cos(lastAngle - BOX_CURVE_ARROW_ANGLE);
            float ay1 = toPos.y - arrowSize * std::sin(lastAngle - BOX_CURVE_ARROW_ANGLE);
            float ax2 = toPos.x - arrowSize * std::cos(lastAngle + BOX_CURVE_ARROW_ANGLE);
            float ay2 = toPos.y - arrowSize * std::sin(lastAngle + BOX_CURVE_ARROW_ANGLE);
