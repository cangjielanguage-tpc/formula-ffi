...
struct Molecule {
    ...
    static float calculateAtomWidth(const std::wstring& atomLabel) {
        ...
        size_t i = 0;
        while (i < atomLabel.size()) {
            wchar_t ch = atomLabel[i];

            if (ch == L'\\') {
                size_t j = i + 1;
                while (j < atomLabel.size() && atomLabel[j] >= L'a' && atomLabel[j] <= L'z') { j++; }
                float charWidth = (j > i + 1) ? 1.0f : 0.8f;
                if (inSubscript || inSuperscript) {
                    width += charWidth * chemfig::SUBSCRIPT_SCALE;
                } else {
                    width += charWidth;
                }
                if (j < atomLabel.size() && atomLabel[j] == L' ') { j++; }
                i = j;
                continue;
            } else if (ch == L'_') {
                inSubscript = true;
