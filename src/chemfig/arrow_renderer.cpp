...
void ArrowRenderer::drawEquilibrium(Graphics2D& g2, const ChemPoint& from,
    ...
    float offset = style.doubleBondOffset * scale / EQUILIBRIUM_OFFSET_DIVISOR;
    float hl = style.headLength * scale;
    float hw = style.headWidth * scale;
    float margin = hl * ARROW_MARGIN_RATIO;
...
void ArrowRenderer::drawHarpRight(Graphics2D& g2, const ChemPoint& from,
    ...
    float slashLen = style.doubleBondOffset * scale * HARPOON_SLASH_RATIO;
    float slashSpacing = slashLen * HARPOON_SLASH_SPACING_RATIO;
    float slashAngle = HARPOON_SLASH_ANGLE_DEG * CHEM_PI / 180.0f;
