...
std::wstring ChemfigParser::collectBalanced(const wchar_t*& p, wchar_t open, wchar_t close) {
    std::wstring result;
    result.reserve(128);
    int depth = 1;
...
void ChemfigParser::skipBalanced(const wchar_t*& p, wchar_t open, wchar_t close) {
    int depth = 1;
    while (peek(p) != L'\0' && depth > 0) {
        if (peek(p) == L'\\' && peekNext(p) != L'\0') {
            p++; p++;
...
std::wstring ChemfigParser::parseBraceArg(const wchar_t*& p) {
    if (peek(p) != L'{') return L"";
    p++;
    return collectBalanced(p, L'{', L'}');
}

bool ChemfigParser::skipBraceArg(const wchar_t*& p) {
    if (peek(p) != L'{') return false;
    p++;
    skipBalanced(p, L'{', L'}');
    return true;
}
...
BondParams ChemfigParser::parseBondParams(const wchar_t*& p, float currentAngle) {
    ...
    for (auto& f : fields) f.reserve(64);
    int fieldIndex = 0;
...
static bool isLatexStyleCommand(const std::wstring& name) {
    return name == L"scriptstyle" || name == L"scriptscriptstyle" ||
           name == L"displaystyle" || name == L"textstyle";
}

static bool isLatexSymbolCommand(const std::wstring& name) {
    return name == L"ominus" || name == L"oplus" ||
           name == L"cdot" || name == L"circ" ||
           name == L"bullet" || name == L"times";
}

static bool isPhantomCommand(const std::wstring& name) {
    return name == L"vphantom" || name == L"hphantom" || name == L"phantom";
}

static bool isTwoBraceCommand(const std::wstring& name) {
    return name == L"chemabove" || name == L"chembelow" || name == L"charge";
}

static wchar_t localPeek(const wchar_t* p, size_t offset = 0) {
    return (p && *(p + offset) != L'\0') ? *(p + offset) : L'\0';
}

static bool readLatexCommandName(const wchar_t*& p, std::wstring& outName) {
    while (localPeek(p) != L'\0' && localPeek(p) >= L'a' && localPeek(p) <= L'z') {
        outName += *p;
        p++;
    }
...
static void appendTwoBraceArgs(const wchar_t*& p, std::wstring& label) {
    for (int i = 0; i < 2; i++) {
        if (localPeek(p) == L'{') {
            label += L'{';
            label += ChemfigParser::parseBraceArg(p);
            label += L'}';
        }
    }
}

static bool processKnownLatexCommand(const wchar_t*& p, std::wstring& label, const std::wstring& cmdName) {
    if (isTwoBraceCommand(cmdName)) {
        label += L'\\';
        label += cmdName;
        appendTwoBraceArgs(p, label);
        return true;
    }
    if (isPhantomCommand(cmdName)) {
        ChemfigParser::skipBraceArg(p);
        return true;
    }
    if (isLatexStyleCommand(cmdName) || isLatexSymbolCommand(cmdName)) {
        label += L'\\';
        label += cmdName;
        return true;
    }
...
bool ChemfigParser::tryProcessLatexCommand(const wchar_t*& p, std::wstring& label) {
    if (peek(p) != L'\\') return false;

    const wchar_t* cmdStart = p;
    p++; // skip '\'
    std::wstring cmdName;
    if (!readLatexCommandName(p, cmdName)) {
        p = cmdStart;
        return false;
    }

    if (processKnownLatexCommand(p, label, cmdName)) return true;
...
std::wstring ChemfigParser::parseAtomGroup(const wchar_t*& p) {
    ...
    while (peek(p) != L'\0') {
        ...
        if (inBraces) {
            ...
            if (ch == L'\\') {
                if (tryProcessLatexCommand(p, label)) continue;
                p++;
                label += ch;
                continue;
            }
...
bool ChemfigParser::parseChargeSpec(const wchar_t*& p, std::vector<Charge>& charges) {
    ...
    std::wstring chargeSpec = collectBalanced(p, L'{', L'}');
        ...
        if (peek(p) == L'\\' && peekNext(p) == L'c') {
            ...
            if (cmd == L"charge") {
                ...
                if (parseChargeSpec(p, charges)) {
                    ...
                    if (match(p, L'{')) {
                        newLabel += collectBalanced(p, L'{', L'}');
                    }
                    continue;
                }
            }
...
bool ChemfigParser::parseDrawCommand(const wchar_t*& p, std::vector<CurvePath>& curves) {
    ...
    if (peek(p) == L'[') {
        p++;
        std::wstring bracketContent = collectBalanced(p, L'[', L']');
    ...
    if (std::abs(cp1.point.distance) < EPSILON && std::abs(cp2.point.distance) < EPSILON &&
    std::abs(cp1.point.angle) < EPSILON && std::abs(cp2.point.angle) < EPSILON) {
        return false;
    }
...
std::shared_ptr<ParseAST> ChemfigParser::parseToAST(const std::wstring& input) {
    auto ast = std::make_shared<ParseAST>();
    ...
    while (peek(p) != L'\0') {
        if (peek(p) == L'\\' && peekNext(p) != L'\0') {
            ...
            if (cmdName == L"chemmove") {
                p = cmdCheck;
                auto dummyMol = std::make_shared<Molecule>();
                parseChemmove(p, *dummyMol);
                for (const auto& curve : dummyMol->curves) {
                    AstCurveNode astCurve;
                    astCurve.fromName = curve.fromName;
                    astCurve.toName = curve.toName;
                    astCurve.controlPoints = curve.controlPoints;
                    astCurve.hasArrow = curve.hasArrow;
                    astCurve.lineWidth = curve.lineWidth;
                    astCurve.shortenStart = curve.shortenStart;
                    astCurve.shortenEnd = curve.shortenEnd;
                    ast->curves.push_back(astCurve);
                }
                skipWhitespace(p);
                continue;
            }
        ...
        auto chain = parseChainToAST(p, atomIndex, false);
        if (chain) {
            ast->topLevelChains.push_back(chain);
        }
...
AstAtomSpec ChemfigParser::parseAtomGroupToAST(const wchar_t*& p) {
    AstAtomSpec spec;
    spec.label = parseAtomGroup(p);
    return spec;
}

bool ChemfigParser::parseChainElementsToAST(const wchar_t*& p, std::vector<AstChainElement>& elements,
    ...
    while (peek(p) != L'\0') {
        ...
        if (ch == L'(') {
            p++;
            AstChainElement elem;
            elem.isBranch = true;
            elem.branchChain = parseChainToAST(p, atomIndex, true);
        ...
        if (ch == L'*') {
            AstChainElement elem;
            elem.isRing = true;
            elem.ringNode = parseRingToAST(p, atomIndex, -1, -1);
            elements.push_back(elem);
            continue;
        }
        ...
        bool hasExplicitBond = (ch == L'-' || ch == L'=' || ch == L'~' || ch == L'>' || ch == L'<');
        AstChainElement elem;
        elem.bondSpec.hasExplicitBond = hasExplicitBond;

        if (hasExplicitBond) {
            elem.bondSpec.type = parseBondType(p);
        }
        ...
        skipWhitespace(p);
        elem.bondSpec.params = parseBondParams(p, 0);
        skipWhitespace(p);
        elem.atomLabel = parseAtomGroupToAST(p);

        skipWhitespace(p);
        if (peek(p) == L'@' && peekNext(p) == L'{') {
            elem.anchorName = parseAnchorName(p);
        }

        skipWhitespace(p);
        if (peek(p) == L'?' && peekNext(p) == L'[') {
            elem.hookName = parseHookName(p);
        }
...
std::shared_ptr<AstChainNode> ChemfigParser::parseChainToAST(const wchar_t*& p, int& atomIndex, bool inBranch) {
    auto chain = std::make_shared<AstChainNode>();
    skipWhitespace(p);
    chain->initialAtom = parseAtomGroupToAST(p);

    skipWhitespace(p);
    if (peek(p) == L'@' && peekNext(p) == L'{') {
        chain->initialAnchorName = parseAnchorName(p);
    }

    skipWhitespace(p);
    if (peek(p) == L'?' && peekNext(p) == L'[') {
        chain->initialHookName = parseHookName(p);
    }

    parseChainElementsToAST(p, chain->elements, atomIndex, inBranch);
...
std::shared_ptr<AstRingNode> ChemfigParser::parseRingToAST(const wchar_t*& p, int& atomIndex,
                                                             int sharedFromAtom, int sharedToAtom) {
    if (!match(p, L'*')) return nullptr;

    auto ring = std::make_shared<AstRingNode>();

    ring->hasInnerCircle = match(p, L'*');
    ring->innerStartAngle = 0;
    ring->innerEndAngle = 360;
    if (ring->hasInnerCircle && match(p, L'[')) {
        ring->innerStartAngle = parseNumber(p);
        if (match(p, L',')) {
            ring->innerEndAngle = parseNumber(p);
        }
        while (peek(p) != L']' && peek(p) != L'\0') p++;
        match(p, L']');
    }
    ...
    ring->sides = 0;
    while (peek(p) >= L'0' && peek(p) <= L'9') {
        ring->sides = ring->sides * 10 + (*p - L'0');
        p++;
    }
    if (ring->sides < 3 || ring->sides > 12) ring->sides = 6;
    if (!match(p, L'(')) return nullptr;

    ring->sharedFromAtomIdx = sharedFromAtom;
    ring->sharedToAtomIdx = sharedToAtom;

    bool isFused = (sharedFromAtom >= 0 && sharedToAtom >= 0);

    int bondCount = 0;
    while (bondCount < ring->sides && peek(p) != L')' && peek(p) != L'\0') {
        int i = bondCount;

        if (isFused && i == ring->sides - 1) {
            skipWhitespace(p);
            AstRingBondSpec fusedSpec;
            fusedSpec.bond.type = parseBondType(p);
            fusedSpec.isFusedRing = true;
            skipWhitespace(p);
            if (peek(p) == L'_' || peek(p) == L'^') p++;
            skipWhitespace(p);
            fusedSpec.bond.params = parseBondParams(p, 0);
            skipWhitespace(p);

            if (peek(p) == L'*') {
                fusedSpec.fusedRing = parseRingToAST(p, atomIndex, sharedToAtom, sharedFromAtom);
                fusedSpec.fusedBondType = fusedSpec.bond.type;
                ring->bondSpecs.push_back(fusedSpec);
                bondCount++;
                continue;
            } else {
                fusedSpec.atomLabel = parseAtomGroupToAST(p);
                ring->bondSpecs.push_back(fusedSpec);
                bondCount++;
                continue;
            }
        ...
        if (peek(p) == L'*') {
            AstRingBondSpec nestedRingSpec;
            nestedRingSpec.isFusedRing = true;
            nestedRingSpec.fusedRing = parseRingToAST(p, atomIndex, -1, -1);
            if (nestedRingSpec.fusedRing) {
                ring->bondSpecs.push_back(nestedRingSpec);
            }
            bondCount++;
            continue;
        }
        ...
        if (peek(p) == L'(') {
            p++;
            auto branchChain = parseChainToAST(p, atomIndex, true);

            if (!ring->bondSpecs.empty()) {
                ring->bondSpecs.back().branches.push_back({branchChain});
            }
            continue;
        }

        AstRingBondSpec bondSpec;
        bondSpec.bond.type = parseBondType(p);
        skipWhitespace(p);
        if (peek(p) == L'_' || peek(p) == L'^') p++;
        skipWhitespace(p);
        bondSpec.bond.params = parseBondParams(p, 0);
        skipWhitespace(p);
        bondSpec.atomLabel = parseAtomGroupToAST(p);

        skipWhitespace(p);
        if (peek(p) == L'?' && peekNext(p) == L'[') {
            bondSpec.hookName = parseHookName(p);
        }

        skipWhitespace(p);
        if (peek(p) == L'@' && peekNext(p) == L'{') {
            bondSpec.anchorName = parseAnchorName(p);
        }
        ...
        if (peek(p) == L'(') {
            p++;
            auto branchChain = parseChainToAST(p, atomIndex, true);
            bondSpec.branches.push_back({branchChain});
        }

        ring->bondSpecs.push_back(bondSpec);
        bondCount++;
    }
