...
enum BondType {
    ...
    BOND_WEDGE_UP = 5,
    BOND_WEDGE_DOWN = 6,
    BOND_WEDGE_DASHED_UP = 7,
    BOND_WEDGE_DASHED_DOWN = 8,
...
struct Bond {
    ...
    Bond() : type(BOND_SINGLE), fromAtom(-1), toAtom(-1) {}
};
...
struct Molecule {
    ...
    int addBond(int from, int to, BondType type, const BondParams& params = BondParams()) {
        int id = bonds.size();
        Bond bond;
        bond.fromAtom = from;
        bond.toAtom = to;
